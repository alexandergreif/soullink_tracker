...
class ResourceManager:
    ...
    def _detect_bundle_environment(self) -> bool:
        ...
        return (
            hasattr(sys, "frozen")  # Nuitka
            or hasattr(sys, "_MEIPASS")  # PyInstaller
            or "site-packages" not in __file__  # Bundled check
        )
    ...
    def run(self):
        ...
        if icon_image:
            self.icon = pystray.Icon(
                "SoulLink Tracker", icon_image, menu=self.create_menu()
            )
            self.icon.run()
...
class SoulLinkLauncher:
    ...
    def check_dependencies(self) -> bool:
        ...
        try:
            ...
            if lua_dir.exists():
                ...
                logging.info(
                    f"Lua directory found: {lua_dir} ({len(lua_files)} .lua files)"
                )
            else:
                logging.warning(f"Lua directory not found: {lua_dir}")

            # Log resource manager environment
            logging.info("Resource Manager Environment:")
    ...
    def setup_environment(self):
        ...
        os.environ["SOULLINK_DATA_DIR"] = str(
            self.resource_manager.get_data_files_directory()
        )
        ...
        os.environ["SOULLINK_PORTABLE"] = (
            "1" if self.resource_manager.is_bundled else "0"
        )
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if self.port is None:
                print(
                    "[ERROR] No available ports found. Please close other applications and try again."
                )
                return 1
