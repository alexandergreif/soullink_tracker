...
class SoulLinkWatcherLauncher:
    ...
    def setup_logging(self) -> None:
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler(
                    self.resource_manager.user_data_dir / "user_launcher.log"
                ),
            ],
        )
        self.logger = logging.getLogger(__name__)
    ...
    def check_dependencies(self) -> bool:
        ...
        if not self.resource_manager.lua_dir.exists():
            self.logger.error(
                f"Lua directory not found: {self.resource_manager.lua_dir}"
            )
            return False
...
'''
    ...
    def start_watcher(self) -> bool:
        ...
        try:
            ...
            config = get_config()
            self.logger.info(
                f"Starting watcher with config: API URL={config.api_url}, Spool Dir={config.spool_dir}"
            )
