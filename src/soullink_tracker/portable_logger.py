...
class PortableLogger:
    ...
    def setup_logging(self):
        ...
        startup_handler = logging.FileHandler(self.startup_log, mode='w', encoding='utf-8')
        ...
        runtime_handler = logging.FileHandler(self.runtime_log, mode='w', encoding='utf-8')
    ...
    def log_critical_imports(self) -> Dict[str, bool]:
        ...
        logger.info(f"Import test complete: {len(results) - failed_count}/{len(results)} successful")
    ...
    def log_resource_check(self, resource_path: Path, resource_type: str = "file") -> bool:
        ...
        if exists:
            if resource_path.is_file():
                ...
                logger.info(f"Resource OK: {resource_type} {resource_path} ({size} bytes)")
            else:
    ...
    def create_diagnostic_dump(self) -> Path:
        ...
        dump_file = self.log_dir / f"diagnostic_dump_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        diagnostic_data = {
            ...
            "environment": {
                ...
                "relevant_vars": {
                    ...
                    ['PATH', 'PYTHONPATH', 'SOULLINK_DEBUG', 'SOULLINK_PORTABLE']
                }
            },
...
def setup_portable_logging(log_dir: Optional[Path] = None, debug: bool = False) -> PortableLogger:
...
def log_startup_error(error: Exception, context: str = "startup"):
    ...
    error_msg = f"{timestamp} STARTUP ERROR in {context}: {type(error).__name__}: {error}\n"
