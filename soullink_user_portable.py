#!/usr/bin/env python3
"""
SoulLink Tracker User Portable Edition - Main Entry Point

This is the entry point for the user portable version of SoulLink Tracker.
This version focuses on the watcher and client components only.

Usage:
    python soullink_user_portable.py        # Run in development
    soullink-tracker-user.exe               # Run as compiled executable
    python soullink_user_portable.py --debug   # Run with debug diagnostics
"""
...
def immediate_startup_log(message: str):
    ...
    log_msg = f"{timestamp} [USER-STARTUP] {message}"
    ...
    try:
        with open("soullink_user_startup_immediate.log", "a", encoding="utf-8") as f:
            f.write(log_msg + "\n")
    except:
...
def emergency_error_log(error: Exception, context: str = "startup"):
    ...
    error_msg = f"""
{timestamp} CRITICAL ERROR in {context} (USER LAUNCHER)
{'='*60}
...
"""
    ...
    try:
        with open("soullink_user_emergency.log", "a", encoding="utf-8") as f:
            f.write(error_msg + "\n")
    except:
...
def setup_environment():
    ...
    try:
        ...
        # Add src directory to Python path for soullink_tracker imports
        ...
        # Add watcher/src directory to Python path for soullink_watcher imports
        watcher_src_dir = bundle_dir / "watcher" / "src"
        if watcher_src_dir.exists():
            sys.path.insert(0, str(watcher_src_dir))
            print(f"Added to Python path: {watcher_src_dir}")
        else:
            print(f"WARNING: watcher/src directory not found at {watcher_src_dir}")
...
def test_critical_imports():
    ...
    critical_modules = [
        ...
        ('soullink_tracker.user_launcher', 'User application launcher'),
        ('soullink_watcher', 'Watcher package'),
        ('soullink_watcher.main', 'Watcher main module'),
...
def run_diagnostics():
    ...
    print("\n" + "="*60)
    print("SoulLink Tracker User Portable - Diagnostic Mode")
    print("="*60)
    ...
    try:
        ...
        logger = setup_portable_logging(debug=True, role="user")
    ...
    print("\n" + "="*60)
    print("Diagnostic complete - ready to run user application")
    print("="*60)
    return True
...
def main():
    ...
    immediate_startup_log("=== SoulLink Tracker User Portable Edition Starting ===")
    ...
    try:
        ...
        if debug_mode:
            ...
            print("SoulLink Tracker User Portable Edition")
            print("Starting watcher and client components...")
        ...
        try:
            from soullink_tracker.portable_logger import setup_portable_logging
            logger = setup_portable_logging(debug=debug_mode, role="user")
        ...
        # Import and run the user launcher
        immediate_startup_log("Importing user launcher...")
        try:
            from soullink_tracker.user_launcher import main as user_launcher_main
            immediate_startup_log("User launcher imported successfully")
            print("Starting SoulLink Tracker User Components...")
            immediate_startup_log("Calling user launcher main function...")
            result = user_launcher_main()
            immediate_startup_log(f"User launcher completed with exit code: {result}")
            return result
            
        except ImportError as e:
            immediate_startup_log(f"FATAL: Could not import user launcher: {e}")
            emergency_error_log(e, "user_launcher_import")
            print(f"\nFATAL: Could not import user launcher: {e}")
        ...
        print("Check soullink_user_emergency.log for detailed error information.")
